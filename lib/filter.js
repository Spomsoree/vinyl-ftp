...
module.exports = {
    ...
    newerOrDifferentSize: function ( folder, options ) {
        ...
        return this.filter( folder, function ( file, remote, cb ) {
            ...
            console.log(file.stat.mtime, remote?.ftp?.date, new Date(file.stat.mtime) > (remote?.ftp?.date ? new Date(remote?.ftp?.date) : null));

            cb( null, !remote || new Date(file.stat.mtime) > new Date(remote.ftp.date) || file.stat.size !== remote.ftp.size );
