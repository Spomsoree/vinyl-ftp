...
module.exports = {
    ...
    newerOrDifferentSize: function ( folder, options ) {
        ...
        return this.filter( folder, function ( file, remote, cb ) {
            ...
            console.log('newer', file.stat.mtime > remote?.ftp?.date);
            console.log('local', file.stat.mtime);
            console.log('remote', remote?.ftp?.date);
            console.log('differentSize', file.stat.size !== remote?.ftp?.size);
